...
int main() {
    ...
    std::cout << "3) Number of Threads per Block: dim3 (16,16,1)" << std::endl;
    std::cout << "4) Number of Blocks per Grid: dim3 " << std::endl;
    std::cout << std::endl;
    ...
    dim3 threadsPerBlock(16, 16, 1);  // 2D
    dim3 blocksPerGrid(
        (sqrt(size) + sqrt(threadsPerBlock.x - 1)) / threadsPerBlock.x,
        (sqrt(size) + sqrt(threadsPerBlock.y - 1)) / threadsPerBlock.y,
        1
    );
