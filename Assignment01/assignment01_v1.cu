...
__global__ void addVectors(double* result, double* vector1, double* vector2, int size) {
    ...
    int j = blockIdx.y * blockDim.y + threadIdx.y; // j=0 always
...
int main() {
    ...
    // int size = 900000000;
    int size = 350000000;
    size_t bytes = size * sizeof(double); // double -> 8 bytes
    ...
    double* deviceResult;
    auto flag_v1 = ::cudaMalloc(&deviceVector1, bytes);
    auto flag_v2 = ::cudaMalloc(&deviceVector2, bytes);
    auto flag_result = ::cudaMalloc(&deviceResult, bytes);
    std::cout << "Time taken for memory allocation on GPU: " << timer.elapsedTime() << " seconds" << std::endl << std::endl;
    std::cout << "Memory allocation on device for vector1: " << flag_v1 << std::endl;
    std::cout << "Memory allocation on device for vector2: " << flag_v1 << std::endl;
    std::cout << "Memory allocation on device for result: " << flag_result << std::endl << std::endl;
